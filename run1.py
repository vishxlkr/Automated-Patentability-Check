...
class PatentResearchSystem:
    ...
    def train_and_evaluate(self, pairs_df):
        ...
        print(f"\nOptimal threshold: {self.threshold:.3f} (F1={best_f1:.3f})")

        # Evaluate on test set
        test_pred = (test_pairs['similarity'] >= self.threshold).astype(int)
        ...
        return {'accuracy': acc, 'precision': prec, 'recall': rec, 'f1': f1,
                'auc': auc, 'threshold': self.threshold, 'confusion_matrix': cm}
    ...
    def plot_results(self, test_pairs, predictions, auc):
        ...
        axes[0].axvline(self.threshold, color='blue', linestyle='--',
