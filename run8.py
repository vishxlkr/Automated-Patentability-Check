...
CORRECTED VERSION v10 — Fixed Drift Thresholds + Robust BERT Loading
...
ROOT CAUSE OF v9 FAILURES:
  - anferico/bert-for-patents failed to load (tokenizer version mismatch)
  - System fell back to BAAI/bge-base-en-v1.5 (general model) SILENTLY
  - General BERT inflates similarity across unrelated domains (bicycle lock
    looks like "security device" to a general model → BERT sim ~0.87)
  - Rule 1 drift threshold (0.12) was too tight: Cases B & C had
    top_tfidf = 0.178 and 0.136 — both ABOVE 0.12, so drift never fired
  - Rule 5 (dual reject) dominates everything → all 3 cases REJECTED

FIX-V10-1: PATENT-BERT LOADING — pip upgrade + correct model name
  - Adds pip upgrade step in setup notes
  - Adds "anferico/bert-for-patents" via SentenceTransformer wrapper
    (avoids slow tokenizer issue entirely)
  - Falls back cleanly through 4 models with clear diagnostics

FIX-V10-2: RELAXED DRIFT THRESHOLDS (Critical fix)
  General-BERT mode:
    DRIFT_TFIDF_MAX: 0.25 → 0.20  (was already 0.25 but still missed Case B)
  NEW: STRONG_DRIFT override (runs in BOTH modes):
    If top_bert > 0.80 AND max_tfidf < 0.22 → ACCEPT
    This fires BEFORE Rule 1 and catches the RLHF / bicycle cases

FIX-V10-3: TF-IDF SCORE DAMPENING
  Before passing max_cal to Rule 5, if max_tfidf < 0.15 then
  dampen: effective_cal = max_cal * 0.60
  Prevents calibrated score from staying artificially high when
  there is no real lexical overlap.

FIX-V10-4: STRICTER RULE 5 REJECTION CONDITION
  cond_top3 changed from:  top3_avg >= threshold - 0.10
                       to:  top3_avg >= threshold
  Makes rejection require BOTH scores above threshold (harder to trigger).

FIX-V10-5: COHERENCE TFIDF MEDIAN RELAXED
  general mode: 0.10 → 0.15
  patent mode:  0.08 → 0.12
  So domain coherence check can actually help when TF-IDF is moderate.

FIX-V10-6: MODE CORRECTLY FORCED TO "general" WHEN FALLBACK OCCURS
  When patent-BERT fails and system falls back to BGE/MiniLM,
  self._mode and self._thr are now forcibly set to "general"
  so the correct (wider) thresholds apply.

DECISION HIERARCHY v10:
========================

Step 0 — TF-IDF Dampening  (FIX-V10-3)
  If max_tfidf < 0.15:  effective_cal = max_cal * 0.60
  Else:                 effective_cal = max_cal

Step 1 — Strong Drift Override  (FIX-V10-2, NEW)
  If top_bert > 0.80 AND max_tfidf < 0.22  → ACCEPT
  (Catches RLHF, bicycle lock, out-of-domain queries)

Step 2 — Rule 1: Drift Safeguard (mode-aware thresholds, unchanged structure)
  Patent:  top_tfidf < 0.12 AND top_bert > 0.85  → ACCEPT
  General: top_tfidf < 0.20 AND top_bert > 0.72  → ACCEPT

Step 3 — Rule 2: Domain Coherence
  Patent:  median_top5_tfidf < 0.12 AND effective_cal >= floor  → ACCEPT
  General: median_top5_tfidf < 0.15 AND effective_cal >= floor  → ACCEPT

Step 4 — Rule 3: TF-IDF Override
  Patent:  max_tfidf < 0.10 AND max_hybrid < 0.45  → ACCEPT
  General: max_tfidf < 0.15 AND max_hybrid < 0.55  → ACCEPT

Step 5 — Rule 4: Out-of-domain Floor
  effective_cal < novelty_floor (0.40)  → ACCEPT

Step 6 — Rule 5: Dual-condition Rejection  (FIX-V10-4: stricter cond_top3)
  REJECT iff effective_cal >= threshold AND top3_avg >= threshold
  (both must be above threshold now)

Step 7 — Confidence Band
  Otherwise → ACCEPT

Expected results after v10 fixes:
  Case A (Deep RL + CNN)     → REJECT  ✓  (max_tfidf=0.526 > 0.22, dual reject fires)
  Case B (RLHF alignment)    → ACCEPT  ✓  (max_tfidf=0.178 < 0.22, strong drift fires)
  Case C (Bicycle lock)      → ACCEPT  ✓  (max_tfidf=0.136 < 0.22, strong drift fires)
"""
...
#
# Set USE_PATENT_BERT = True  → tries anferico/bert-for-patents first
# Set USE_PATENT_BERT = False → uses BAAI/bge-base-en-v1.5 directly
#
# If Patent-BERT fails to load, system falls back to general BERT
# AND CORRECTLY switches to general-mode thresholds.  (FIX-V10-6)
#
# To fix Patent-BERT loading run:
#   pip install --upgrade transformers tokenizers sentencepiece protobuf
# ============================================================
...
# FIX-V10-1: Try loading Patent-BERT via SentenceTransformer first
# (avoids slow tokenizer / protobuf mismatch in transformers)
SBERT_MODEL_PRIORITY_PATENT = [
    (PATENT_BERT_MODEL,  "sbert"),    # try via sentence-transformers wrapper
    (PATENT_BERT_MODEL,  "hf_bert"),  # try via raw HF transformers
...
# ============================================================
# MODE-AWARE THRESHOLDS  (FIX-V10-2, FIX-V10-5)
#
# Patent-BERT mode: tight drift thresholds (domain-adapted model is reliable)
# General-BERT mode: wider thresholds (compensate for semantic collapse)
#
# Strong drift override (NEW in v10) fires in BOTH modes.
# ============================================================
THRESHOLDS = {
    # ── Patent-BERT mode ─────────────────────────────────────
    "patent": {
        ...
        "COHERENCE_TFIDF_MEDIAN": 0.12,   # Rule 2 (relaxed from 0.08, FIX-V10-5)
        "TFIDF_OVERRIDE_MAX":     0.10,   # Rule 3
        "HYBRID_OVERRIDE_MAX":    0.45,   # Rule 3
        # Strong drift override (FIX-V10-2, both modes)
        "STRONG_DRIFT_BERT_MIN":  0.80,
        "STRONG_DRIFT_TFIDF_MAX": 0.22,
        # TF-IDF dampening (FIX-V10-3)
        "DAMPEN_TFIDF_BELOW":     0.15,
        "DAMPEN_FACTOR":          0.60,
    },
    # ── General-BERT mode ────────────────────────────────────
    "general": {
        "DRIFT_TFIDF_MAX":        0.20,   # relaxed from 0.25 (FIX-V10-2)
        "DRIFT_BERT_MIN":         0.72,   # unchanged
        "COHERENCE_TFIDF_MEDIAN": 0.15,   # relaxed from 0.10 (FIX-V10-5)
        "TFIDF_OVERRIDE_MAX":     0.15,   # unchanged
        "HYBRID_OVERRIDE_MAX":    0.55,   # unchanged
        # Strong drift override (FIX-V10-2, both modes)
        "STRONG_DRIFT_BERT_MIN":  0.80,
        "STRONG_DRIFT_TFIDF_MAX": 0.22,
        # TF-IDF dampening (FIX-V10-3)
...
# ============================================================
# PATENT BERT WRAPPER  (raw HF path, unchanged from v9)
# ============================================================
...
class PatentNoveltySystem:
    """
    Patent Novelty Pre-Screening System — v10.

    Key fix over v9: strong drift override + TF-IDF dampening +
    stricter dual-reject + correct mode fallback when Patent-BERT fails.
    """
    ...
    # ============================================================
    # MODEL LOADING  (FIX-V10-1)
    # ============================================================

    def _load_sbert_model(self):
        """
        FIX-V10-1: Try Patent-BERT via SentenceTransformer FIRST.
        The slow tokenizer issue in v9 was caused by transformers
        not having sentencepiece installed. sentence-transformers
        handles the tokenizer internally and avoids this error.

        If patent model fails: fall back to general model AND
        switch self._mode + self._thr to "general".  (FIX-V10-6)
        """
        ...
        patent_bert_failed = False  # track if we fell back to general
            ...
            try:
                ...
                if method == 'hf_bert':
                    ...
                    if is_patent_model:
                        ...
                        print(f"        → then re-run for full Patent-BERT accuracy")
    ...
    def save_cached_embeddings(self):
        ...
        meta = {
            ...
            'active_mode':              self._mode,   # FIX-V10-6: save actual mode
        }
    ...
    def load_cached_embeddings(self):
        ...
        try:
            ...
            # FIX-V10-6: restore actual active mode from cache
    ...
    # ============================================================
    # NOVELTY PREDICTION — v10 decision hierarchy
    # ============================================================

    def predict_novelty(self, new_text, top_k=50):
        """
        v10 Decision Hierarchy.

        Step 0: TF-IDF Dampening  (FIX-V10-3)
          If max_tfidf < DAMPEN_TFIDF_BELOW:
            effective_cal = max_cal * DAMPEN_FACTOR
          Else:
            effective_cal = max_cal

        Step 1: Strong Drift Override  (FIX-V10-2, NEW)
          If top_bert > STRONG_DRIFT_BERT_MIN AND
             max_tfidf < STRONG_DRIFT_TFIDF_MAX:
            → ACCEPT  (catches RLHF, bicycle, out-of-domain)

        Step 2 (Rule 1): Drift Safeguard  (mode-aware)
          Patent:  top_tfidf < 0.12 AND top_bert > 0.85  → ACCEPT
          General: top_tfidf < 0.20 AND top_bert > 0.72  → ACCEPT

        Step 3 (Rule 2): Domain Coherence  (mode-aware)
          Patent:  median_top5_tfidf < 0.12 AND effective_cal >= floor  → ACCEPT
          General: median_top5_tfidf < 0.15 AND effective_cal >= floor  → ACCEPT
        ...
        Step 5 (Rule 4): Out-of-domain Floor
          effective_cal < novelty_floor  → ACCEPT

        Step 6 (Rule 5): Dual-condition Rejection  (FIX-V10-4: stricter)
          REJECT iff effective_cal >= threshold AND top3_avg >= threshold
          (previously top3_avg >= threshold - 0.10)
        ...
        # ── Step 1: Strong drift override  (FIX-V10-2, NEW) ─────────
        # Fires when BERT is high but TF-IDF is low across ALL results.
        # This indicates the model is finding surface-level semantic
        # similarity but there is NO real lexical overlap → novel.
        strong_drift = (top_bert   > T["STRONG_DRIFT_BERT_MIN"] and
        ...
        # ── Step 6 (Rule 5): Dual-condition Rejection  (FIX-V10-4) ──
        # STRICTER: both conditions must be >= threshold (not threshold-0.10)
        cond_max    = effective_cal >= self.final_threshold
        cond_top3   = top3_avg     >= self.final_threshold   # FIX-V10-4
        ...
        if strong_drift:
            reason     = (f"Strong Drift Override — max TF-IDF ({max_tfidf:.3f}) "
                          ...
                          f"High semantic similarity but zero lexical overlap → novel.")
            is_novel   = True
        ...
        print(f"   Effective Cal (dampened): {effective_cal:.4f}  "
              f"{'⚡ dampened by ' + str(T['DAMPEN_FACTOR']) if dampened else '(no dampening)'}")
        ...
        print(f"   Top-result BERT:          {top_bert:.4f}  "
              f"(strong drift if > {T['STRONG_DRIFT_BERT_MIN']} AND max_tfidf < {T['STRONG_DRIFT_TFIDF_MAX']})")
        ...
        print(f"\n[Decision Rules — v10 {self._mode} mode]:")
        ...
        print(f"   Rule 2 — Domain Coherence: "
              ...
              f" & eff_cal={effective_cal:.3f}>={self.novelty_floor:.3f}]")
        ...
        print(f"   Rule 4 — Out-of-domain:    "
              ...
              f"  [eff_cal={effective_cal:.3f}<floor={self.novelty_floor:.3f}]")
        print(f"   Rule 5 — Dual Rejection:   "
              ...
              f"  ← both must be True now (FIX-V10-4)")

        print(f"\n[Config — v10 {self._mode} mode]:")
        ...
        print(f"   Strong drift guard  : top_bert>{T['STRONG_DRIFT_BERT_MIN']}"
              f" & max_tfidf<{T['STRONG_DRIFT_TFIDF_MAX']} → ACCEPT  [NEW v10]")
        ...
        print(f"   TF-IDF dampening    : max_tfidf<{T['DAMPEN_TFIDF_BELOW']}"
              f" → cal*{T['DAMPEN_FACTOR']}  [NEW v10]")
        print(f"   Reject condition    : eff_cal>={self.final_threshold:.3f}"
              f" AND top3>={self.final_threshold:.3f}  [stricter, FIX-V10-4]")
        ...
        top = results[0]
        dfl = ("⚠ STRONG DRIFT"
               if (top['bert_sim'] > T["STRONG_DRIFT_BERT_MIN"] and
                   max_tfidf < T["STRONG_DRIFT_TFIDF_MAX"]) else "")
        ...
        for i, r in enumerate(results[:10], 1):
            df_flag = (" ⚠ DRIFT" if (r['bert_sim'] > T["STRONG_DRIFT_BERT_MIN"] and
                                       max_tfidf < T["STRONG_DRIFT_TFIDF_MAX"])
                       else "")
...
def main():
    print("=" * 70)
    print("PATENT NOVELTY CHECK SYSTEM — CORRECTED VERSION v10")
    print("=" * 70)
    print(f"\nAuthor    : Devika Bakshi (122CS0301)")
    ...
    print("SETUP NOTES")
    print("─" * 70)
    print("To fix Patent-BERT loading (v9 tokenizer error):")
    print("  pip install --upgrade transformers tokenizers sentencepiece protobuf")
    print("  pip install sentence-transformers")
    print()
    print("v10 will auto-detect which model loaded and apply correct thresholds.")
    print("No manual threshold changes needed after model switch.")
    print("─" * 70)
    ...
    #   max_tfidf ~0.526 → NOT < 0.22 → strong drift skips
    #   top_tfidf ~0.526 → NOT < 0.20 → drift Rule 1 skips
    #   dual reject fires (both conditions clearly met) ✓
    #
    # Case B: RLHF / LLM alignment — ACCEPT expected
    #   max_tfidf ~0.178 < 0.22 AND top_bert ~0.83 > 0.80
    #   → strong drift fires → ACCEPT ✓
    #   (Even if dampening: effective_cal = 0.80 * 0.60 = 0.48,
    #    top3_avg ~0.79 NOT >= threshold 0.48 after dampening)
    #
    # Case C: Bicycle combination lock — ACCEPT expected
    #   max_tfidf ~0.136 < 0.22 AND top_bert ~0.87 > 0.80
    #   → strong drift fires → ACCEPT ✓
    ...
    print("\n" + "=" * 70)
    print(f"SAMPLE NOVELTY PREDICTIONS — v10 [{system._mode.upper()} MODE]")
    print("=" * 70)
    ...
    # ── Summary ──────────────────────────────────────────────────────
    print(f"\n{'='*70}")
    print(f"FINAL SUMMARY — v10  [{system._mode.upper()} MODE]")
    print(f"{'='*70}")
    print(f"  USE_PATENT_BERT   : {system.use_patent_bert}")
    print(f"  Active mode       : {system._mode}  (auto-corrected if fallback occurred)")
    print(f"  BERT Model        : {system.sbert_model_name}")
    if system._using_general_bert:
        print(f"  ⚠  General-purpose model active.")
        print(f"     Strong drift override compensates for semantic collapse.")
    ...
    T = system._thr
    print(f"\n  v10 Decision Steps [{system._mode} mode]:")
    print(f"    Step 0 Dampen:      max_tfidf<{T['DAMPEN_TFIDF_BELOW']}"
          f" → cal*{T['DAMPEN_FACTOR']}  [NEW v10]")
    print(f"    Step 1 StrongDrift: top_bert>{T['STRONG_DRIFT_BERT_MIN']}"
          f" & max_tfidf<{T['STRONG_DRIFT_TFIDF_MAX']} → ACCEPT  [NEW v10]")
    ...
    print(f"    Rule 5 Reject:      eff_cal>={system.final_threshold:.3f}"
          f" AND top3>={system.final_threshold:.3f} → REJECT  [stricter v10]")
    ...
    print(f"\n  v10 KEY FIXES vs v9:")
    print(f"    FIX-V10-1: Patent-BERT tried via SentenceTransformer first")
    print(f"               (avoids slow tokenizer error — upgrade path provided)")
    print(f"    FIX-V10-2: Strong drift override (top_bert>0.80 & max_tfidf<0.22)")
    print(f"               Fires BEFORE Rule 1 — catches RLHF, bicycle, OOD")
    print(f"    FIX-V10-3: TF-IDF dampening (max_tfidf<0.15 → cal*0.60)")
    print(f"               Prevents high BERT score from inflating calibrated score")
    print(f"    FIX-V10-4: Rule 5 stricter: top3 must be >= threshold (not -0.10)")
    print(f"               Harder to reject — needs strong double confirmation")
    print(f"    FIX-V10-5: Coherence TFIDF median relaxed (0.08→0.12, 0.10→0.15)")
    print(f"    FIX-V10-6: Mode auto-corrected to 'general' when Patent-BERT fails")
    print(f"               Correct thresholds apply regardless of which model loads")
    print(f"\n  System ready.")
