...
STABLE VERSION v13 — MPNet Semantic Model + Three-Way Decision
...
WHAT v13 ADDS vs v12:
======================

CHANGE-V13-1: THREE-WAY DECISION
  Old:  [ACCEPT] Novel  /  [REJECT] Not Novel
  New:  [ACCEPT] Novel  /  [BORDERLINE] Incremental Innovation  /  [REJECT] Prior Art

  The incremental zone captures patents that are closely related to prior art
  but still show a distinct improvement pattern — high lexical + semantic overlap,
  but a consistency and gap signature that differs from direct copying.

CHANGE-V13-2: DYNAMIC INCREMENTAL THRESHOLDS (data-driven, not hardcoded)
  During training, we collect the joint score distribution of positive citation pairs:
    - max_tfidf distribution → set INCREMENTAL_TFIDF_LOW  at 75th percentile
    - top3_avg distribution  → set INCREMENTAL_CAL_LOW    at 75th percentile
    - semantic_gap distribution → set INCREMENTAL_GAP_HIGH at 25th percentile
                                  (small gap = high coherence = incremental, not drifted)
  Then during training calibration we also record:
    - INCREMENTAL_CAL_HIGH  = final_threshold (upper boundary: above this = REJECT)
    - INCREMENTAL_TFIDF_HIGH = 90th percentile of positive tfidf scores

  Incremental zone: score is HIGH (above 75th pct of positives) but below full REJECT,
  AND the semantic gap is SMALL (coherent domain match, not spurious semantic drift),
  AND tfidf is meaningfully non-zero (actual lexical overlap, not noise).

  Why this is correct:
  - Positive pairs in citation data = patents that were actually cited by later patents
  - A patent cited by another is related but not identical
  - The upper tail of cited-pair scores = the "similar but distinct" regime
  - That is exactly the incremental innovation zone

CHANGE-V13-3: INCREMENTAL SITS INSIDE REJECT TERRITORY (logically correct)
  The incremental check runs BEFORE dual_reject in the decision hierarchy.
  This means: a query that would have been REJECTED gets BORDERLINE instead
  if its score fingerprint matches the incremental pattern.
  Queries well below threshold still get ACCEPT normally.
  Only the dual_reject zone is split into REJECT + BORDERLINE.

CHANGE-V13-4: METRICS IMPACT IS MINIMAL
  - TF-IDF matrix, MPNet embeddings, cosine similarities: unchanged
  - Hybrid scores, calibrated scores: unchanged
  - Training, AUC, retrieval Recall@K: unchanged
  - Only the final label for edge-case queries changes:
      some former REJECTs become BORDERLINE (treated as is_novel=True for binary metrics)
  - For binary accuracy reporting, BORDERLINE counts as ACCEPT (novel)
    which may slightly lower precision but is the legally correct interpretation

DECISION HIERARCHY v13:
========================
Step 0  — TF-IDF Dampening
Step 1  — Modern Terminology Override   → ACCEPT
Step 2  — Semantic Gap Override         → ACCEPT
Step 3  — Strong Drift Override         → ACCEPT
Step 4  — TF-IDF Rank Decay            → ACCEPT
Step 5  — Rule 1: Drift Safeguard      → ACCEPT
Step 6  — Rule 2: Domain Coherence     → ACCEPT
Step 7  — Rule 3: TF-IDF Override      → ACCEPT
Step 8  — Rule 4: Out-of-domain Floor  → ACCEPT
Step 9  — [NEW] Incremental Innovation → BORDERLINE  ← added here, before REJECT
Step 10 — Rule 5: Dual Rejection       → REJECT
Step 11 — Confidence Band              → ACCEPT
"""
...
# ============================================================
# THRESHOLDS (v13 — same as v12 plus incremental zone params)
# ============================================================
THRESHOLDS = {
    ...
    # Semantic gap override (with score guard)
    ...
    # Incremental zone — these are INITIAL defaults; overwritten dynamically at train time
    # They represent the lower boundary of the incremental zone.
    # Upper boundary = final_threshold (same as REJECT cutoff).
    # If score is in [INCREMENTAL_CAL_LOW, final_threshold] AND gap is small
    # AND tfidf is high enough → BORDERLINE instead of REJECT.
    "INCREMENTAL_CAL_LOW":        0.60,   # overwritten by 75th pct of pos cal scores
    "INCREMENTAL_TFIDF_LOW":      0.18,   # overwritten by 75th pct of pos tfidf scores
    "INCREMENTAL_GAP_HIGH":       0.35,   # overwritten by 25th pct of pos gap scores (upper bound on gap)
    "INCREMENTAL_TFIDF_HIGH":     0.40,   # overwritten by 90th pct of pos tfidf scores
    "INCREMENTAL_TOP3_LOW":       0.60,   # overwritten by 75th pct of pos top3 scores
}
...
class PatentNoveltySystem:
    """
    Hybrid Patent Novelty Detection System — v13
    ...
    Architecture:
      ...
      - [NEW v13] Incremental zone: data-driven BORDERLINE decision
        for patents that are closely related but show improvement patterns

    Three-way output:
      [ACCEPT]     → Potentially Novel (clear novelty)
      [BORDERLINE] → Incremental Innovation (improvement on prior art)
      [REJECT]     → Prior Art Detected (not novel)
    """
    ...
    def __init__(self, cache_dir='cache/', model_dir='models/'):
        ...
        self._thr        = dict(THRESHOLDS)  # mutable copy — updated at train time
        ...
        # v13: incremental zone score distributions (set during training)
    ...
    def train_hybrid_model(self, positive_pairs):
        ...
        # ── v13: Learn incremental zone thresholds from positive pair distribution ──
    ...
    def _learn_incremental_thresholds(self, tt, st, yl, vt, vs, vl, vp, vla):
        """
        v13: Derive the incremental zone boundaries from the actual score
        distributions of citation-positive pairs (cited patents = related, novel enough
        to be granted but still cited = the definition of incremental improvement).

        The incremental zone is the HIGH end of positive pair scores:
          - Pairs that scored above the 75th percentile of positives are
            "very similar but still cited" = incremental innovation territory
          - The upper bound is final_threshold (above = REJECT)
          - The gap constraint ensures we're not seeing drift, just lexical similarity

        We use both training AND validation positives for better statistics.
        """
        print(f"\n[v13] Learning incremental zone thresholds from positive pair distribution...")

        # Collect positive scores from training and validation sets
        pos_tfidf   = [tt[i] for i in range(len(yl)) if yl[i] == 1]
        pos_semantic = [st[i] for i in range(len(yl)) if yl[i] == 1]
        pos_gap     = [abs(st[i] - tt[i]) for i in range(len(yl)) if yl[i] == 1]
        pos_fused   = [self._fuse(t, s) for t, s in zip(pos_tfidf, pos_semantic)]

        val_pos_tfidf    = [vt[i] for i in range(len(vl)) if vl[i] == 1]
        val_pos_semantic = [vs[i] for i in range(len(vl)) if vl[i] == 1]
        val_pos_gap      = [abs(vs[i] - vt[i]) for i in range(len(vl)) if vl[i] == 1]

        # Calibrated scores for val positives
        val_pos_cal = [float(vp[i]) for i in range(len(vla)) if vla[i] == 1]

        # Combine
        all_pos_tfidf   = pos_tfidf   + val_pos_tfidf
        all_pos_semantic = pos_semantic + val_pos_semantic
        all_pos_gap     = pos_gap     + val_pos_gap

        if len(all_pos_tfidf) < 10:
            print(f"   Too few positive pairs ({len(all_pos_tfidf)}) — using defaults")
            return

        # Compute percentile-based boundaries
        # TFIDF lower bound: 75th percentile of positive pair TF-IDF scores
        # Rationale: the top 25% of positives by TF-IDF are the most lexically similar
        tfidf_p75  = float(np.percentile(all_pos_tfidf, 75))
        tfidf_p90  = float(np.percentile(all_pos_tfidf, 90))

        # Gap upper bound: 25th percentile of gap (small gap = coherent domain)
        # We want gap to be SMALL for incremental (not drifted)
        # So we take the 25th percentile — most positives have smaller gaps than this
        gap_p25 = float(np.percentile(all_pos_gap, 25))
        gap_p50 = float(np.percentile(all_pos_gap, 50))

        # Calibrated score lower bound: 75th percentile of val positive calibrated scores
        # Rationale: incremental zone = top 25% of positive scores (below REJECT threshold)
        if len(val_pos_cal) >= 5:
            cal_p75 = float(np.percentile(val_pos_cal, 75))
        else:
            # Fall back to fused score distribution
            cal_p75 = float(np.percentile(pos_fused, 75))

        # Top3 lower bound: same reasoning
        top3_p75 = cal_p75  # approximation; actual top3 not available at training time

        # Safety clamps: incremental lower bound must be below final_threshold
        cal_p75  = min(cal_p75,  self.final_threshold - 0.02)
        tfidf_p75 = max(tfidf_p75, 0.08)  # must be non-trivial TF-IDF

        self._thr["INCREMENTAL_CAL_LOW"]    = cal_p75
        self._thr["INCREMENTAL_TFIDF_LOW"]  = tfidf_p75
        self._thr["INCREMENTAL_TFIDF_HIGH"] = tfidf_p90
        self._thr["INCREMENTAL_GAP_HIGH"]   = gap_p50   # use median gap as ceiling
        self._thr["INCREMENTAL_TOP3_LOW"]   = cal_p75

        self._incremental_thresholds_learned = True

        print(f"\n   Positive pair score distribution (n={len(all_pos_tfidf)}):")
        print(f"     TF-IDF   — p25={np.percentile(all_pos_tfidf, 25):.3f}  "
              f"p50={np.percentile(all_pos_tfidf, 50):.3f}  "
        ...
        print(f"     Gap      — p25={gap_p25:.3f}  p50={gap_p50:.3f}  "
              f"p75={np.percentile(all_pos_gap, 75):.3f}")
        if val_pos_cal:
            print(f"     Cal(val) — p25={np.percentile(val_pos_cal, 25):.3f}  "
                  f"p50={np.percentile(val_pos_cal, 50):.3f}  "
                  f"p75={cal_p75:.3f}")
        print(f"\n   Incremental zone thresholds (learned):")
        print(f"     INCREMENTAL_CAL_LOW    = {self._thr['INCREMENTAL_CAL_LOW']:.3f}  "
              f"(75th pct of pos cal scores; lower boundary of incremental zone)")
        print(f"     INCREMENTAL_TFIDF_LOW  = {self._thr['INCREMENTAL_TFIDF_LOW']:.3f}  "
              f"(75th pct of pos TF-IDF; must have real lexical overlap)")
        print(f"     INCREMENTAL_TFIDF_HIGH = {self._thr['INCREMENTAL_TFIDF_HIGH']:.3f}  "
              f"(90th pct of pos TF-IDF; upper TF-IDF bound)")
        print(f"     INCREMENTAL_GAP_HIGH   = {self._thr['INCREMENTAL_GAP_HIGH']:.3f}  "
              f"(median pos gap; gap must be BELOW this → coherent domain)")
        print(f"     Upper boundary         = final_threshold = {self.final_threshold:.3f}")
        print(f"\n   Interpretation:")
        print(f"     Calibrated score in [{self._thr['INCREMENTAL_CAL_LOW']:.3f}, "
              f"{self.final_threshold:.3f}]")
        print(f"     AND TF-IDF in [{self._thr['INCREMENTAL_TFIDF_LOW']:.3f}, "
              f"{self._thr['INCREMENTAL_TFIDF_HIGH']:.3f}]")
        print(f"     AND semantic gap < {self._thr['INCREMENTAL_GAP_HIGH']:.3f}")
        print(f"     → [BORDERLINE] Incremental Innovation")
    ...
    def evaluate_model(self, eval_pairs, eval_top_k=100):
        ...
        print(f"   [v13] BORDERLINE counts as ACCEPT (novel) for binary metrics")
    ...
    def save_cached_embeddings(self):
        ...
        meta = {
            ...
            'thresholds':                     self._thr,           # v13: save full threshold dict
    ...
    def load_cached_embeddings(self):
        ...
        try:
            ...
            # Restore full threshold dict including learned incremental zone values
            if 'thresholds' in m:
            ...
            logger.info(f"Cache loaded (v13, model={SEMANTIC_MODEL_NAME})")
            return True
        except Exception as e:
    ...
    # ============================================================
    # NOVELTY PREDICTION — v13 decision hierarchy
    # ============================================================

    def predict_novelty(self, new_text, top_k=50):
        """
        v13 Decision Hierarchy:
        ...
        Step 7:  Rule 3 — TF-IDF Override         → ACCEPT
        Step 8:  Rule 4 — Out-of-domain Floor     → ACCEPT
        Step 9:  [NEW] Incremental Innovation     → BORDERLINE  ← before REJECT
        Step 10: Rule 5 — Dual Rejection          → REJECT
        Step 11: Confidence Band                  → ACCEPT

        BORDERLINE logic (data-driven, set at train time):
        Score is in the HIGH zone of positive pair distribution but below REJECT,
        AND gap is SMALL (coherent domain, not drifted),
        AND TF-IDF is meaningfully non-zero (real lexical overlap).
        This is the "improvement patent" fingerprint.
        """
        if self.patents_df is None:
            raise ValueError("Dataset not loaded.")

        print("\n" + "=" * 70)
        print("NOVELTY PREDICTION — Hybrid TF-IDF + MPNet System v13")
        print("=" * 70)
        ...
        # ── Step 9 [v13]: Incremental Innovation ──────────────────────
        # Triggered BEFORE dual_reject so incremental patents don't get REJECT.
        # Conditions (all data-driven from training positive pair distribution):
        #   1. Calibrated score is in the HIGH zone of positive pairs
        #      → effective_cal in [INCREMENTAL_CAL_LOW, final_threshold]
        #      → this is below REJECT but above what most non-cited pairs score
        #   2. TF-IDF is in the meaningful overlap range
        #      → [INCREMENTAL_TFIDF_LOW, INCREMENTAL_TFIDF_HIGH]
        #      → ensures there IS lexical overlap (not noise or domain drift)
        #   3. Semantic gap is SMALL
        #      → gap < INCREMENTAL_GAP_HIGH (median gap of positive pairs)
        #      → small gap = patent is in same semantic domain, not drifted
        #      → this distinguishes "improvement in same domain" from
        #         "different technology that sounds similar"
        #   4. top3_avg is also high (consistent top-3, not single spurious match)
        incremental_patent = (
            ...
            semantic_gap < T["INCREMENTAL_GAP_HIGH"] and
        ...
        if modern_override:
            ...
            # The incremental zone: high enough to be related, low enough to be distinct.
            # TF-IDF shows real lexical overlap (same field), semantic gap is small
            # (same domain, not drift), but score doesn't cross REJECT threshold.
            # Classic improvement/optimization patent signature.
            gap_abs = abs(effective_cal - self.final_threshold)
            reason  = (
                f"Incremental Innovation Pattern — Calibrated score "
                f"({effective_cal:.3f}) is in the high-similarity zone "
                f"[{T['INCREMENTAL_CAL_LOW']:.3f}–{self.final_threshold:.3f}] "
                f"of citation-positive pairs with meaningful TF-IDF overlap "
                f"({max_tfidf:.3f}) and a small semantic gap ({semantic_gap:.3f} "
                f"< {T['INCREMENTAL_GAP_HIGH']:.3f}). "
                f"Pattern matches improvement/optimization patents rather than "
                f"direct duplication. Manual examiner review recommended."
            )
            is_novel   = True  # legally: improvement patents are grantable
            decision   = "[BORDERLINE] Incremental Innovation"
            confidence = ("Medium-High" if gap_abs > 0.05 else "Medium — manual review required")

        elif dual_reject:
            gap      = abs(effective_cal - self.final_threshold)
            reason   = "Strong prior art match found"
            is_novel = False
            decision = "[REJECT] Not Novel — Prior Art Detected"
        ...
        if self._incremental_thresholds_learned:
            print(f"   Step 9 — Incremental:        "
                  f"{'⚡ → BORDERLINE' if incremental_patent else '○ skip'}"
                  ...
                  f" & gap={semantic_gap:.3f} < {T['INCREMENTAL_GAP_HIGH']:.3f}"
                  f" & top3={top3_avg:.3f} >= {T['INCREMENTAL_TOP3_LOW']:.3f}]")
        else:
            print(f"   Step 9 — Incremental:        ○ skip  [thresholds not yet learned]")
...
def main():
    print("=" * 70)
    print("HYBRID PATENT NOVELTY DETECTION SYSTEM — v13")
    print("TF-IDF + MPNet Semantic Embeddings + Rule-Based Overrides")
    print("THREE-WAY: [ACCEPT] | [BORDERLINE] Incremental | [REJECT]")
    print("=" * 70)
    print(f"\nAuthor    : Devika Bakshi (122CS0301)")
    ...
    print(f"  → v13: Incremental zone thresholds learned from citation data")
    ...
    test_cases = {
        ...
        "D — CNN with attention (EXPECT: BORDERLINE — known field, marginal improvement)": """
    ...
    print("\n" + "=" * 70)
    print("SAMPLE NOVELTY PREDICTIONS — v13")
    print("=" * 70)
    ...
    T = system._thr
    print(f"\n{'='*70}")
    print(f"FINAL SUMMARY — Hybrid Patent Novelty Detection System v13")
    print(f"{'='*70}")
    ...
    print(f"\n  Three-way Decision Output [v13]:")
    print(f"    [ACCEPT]     → Potentially Novel (clear novelty)")
    print(f"    [BORDERLINE] → Incremental Innovation (improvement on prior art)")
    print(f"    [REJECT]     → Prior Art Detected (not novel)")
    ...
    if system._incremental_thresholds_learned:
        print(f"    Step 9  Incremental:  eff_cal in [{T['INCREMENTAL_CAL_LOW']:.3f},"
              f"{system.final_threshold:.3f}]")
        print(f"                          & tfidf in [{T['INCREMENTAL_TFIDF_LOW']:.3f},"
              f"{T['INCREMENTAL_TFIDF_HIGH']:.3f}]")
        print(f"                          & gap < {T['INCREMENTAL_GAP_HIGH']:.3f}"
              f" & top3 >= {T['INCREMENTAL_TOP3_LOW']:.3f} → BORDERLINE")
    else:
        print(f"    Step 9  Incremental:  [not yet learned — run train_hybrid_model first]")
    print(f"    Rule 5  Reject:       eff_cal>={system.final_threshold:.3f}"
          f" AND top3>={system.final_threshold:.3f} → REJECT")

    if metrics:
        print(f"\n  Test Metrics (BORDERLINE counted as ACCEPT for binary metrics):")
        print(f"    Accuracy    : {metrics['accuracy']:.3f}  ({metrics['accuracy']*100:.1f}%)")
        print(f"    Bal. Acc.   : {metrics['balanced_accuracy']:.3f}")
        print(f"    Precision   : {metrics['precision']:.3f}")
        print(f"    Recall      : {metrics['recall']:.3f}")
    ...
    for label, d in decisions.items():
        ...
        if "REJECT" in label and "REJECT" in d:
            ok, mark = True, "✓"
        elif "ACCEPT" in label and ("ACCEPT" in d or "BORDERLINE" in d):
            ok, mark = True, "✓"
        elif "BORDERLINE" in label and "BORDERLINE" in d:
            ok, mark = True, "✓"
        else:
            ok, mark = False, "✗"
    ...
    print(f"\n  v13 changes vs v12:")
    print(f"    CHANGE-V13-1: Three-way decision: ACCEPT / BORDERLINE / REJECT")
    print(f"    CHANGE-V13-2: Incremental thresholds learned from citation pair distribution")
    print(f"                  (not hardcoded — adapts to your actual dataset)")
    print(f"    CHANGE-V13-3: Incremental zone sits inside former REJECT territory")
    print(f"                  (carve-out before dual_reject, so valid improvements not rejected)")
    print(f"    CHANGE-V13-4: Incremental conditions are jointly necessary:")
    print(f"                  score in high-pos-percentile range + real TF-IDF overlap")
    print(f"                  + small semantic gap (same domain, not drift)")
    print(f"    CHANGE-V13-5: Cache version bumped to v13 (auto-recomputes old caches)")
    print(f"\n  System ready.")
