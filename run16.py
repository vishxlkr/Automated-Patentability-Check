...
class PatentNoveltySystem:
    ...
    def build_citation_dataset(self, patent_file, abstract_file, citation_file,
        ...
        # FIX: USPTO TSV files can be multi-GB.  The C tokenizer crashes with
        # "out of memory" on malformed rows; switch to python engine + skip bad
        # lines.  Also peek at headers first so we only read the 2 cols we need.
        core_set = set(core)
        ...
        # Identify title column name from header so we can pass usecols
        try:
            pat_header = _peek_cols(patent_file)
            title_col_hint = next((c for c in pat_header if 'title' in c.lower()), None)
            use_pat = (['patent_id', title_col_hint]
                       if title_col_hint and 'patent_id' in pat_header else None)
        except Exception:
            use_pat = None

        chunks = []
        pat_kwargs = dict(sep='\t', dtype=str, chunksize=50000,
                          on_bad_lines='skip', engine='python')
        if use_pat:
            pat_kwargs['usecols'] = use_pat
        for chunk in pd.read_csv(patent_file, **pat_kwargs):
        ...
        try:
            ...
            abs_col_hint = next((c for c in abs_header if 'abstract' in c.lower()), None)
            use_abs = (['patent_id', abs_col_hint]
                       if abs_col_hint and 'patent_id' in abs_header else None)
        except Exception:
            use_abs = None

        abs_chunks = []
        abs_kwargs = dict(sep='\t', dtype=str, chunksize=50000,
                          on_bad_lines='skip', engine='python')
        if use_abs:
            abs_kwargs['usecols'] = use_abs
        for chunk in pd.read_csv(abstract_file, **abs_kwargs):
            if 'patent_id' not in chunk.columns:
                continue
