...
CORRECTED VERSION v7 — Semantic Drift Safeguard + BERT-Free Decision Logic

Author: Devika Bakshi (122CS0301)
Supervisor: Asst. Prof. Sumanta Pyne
NIT Rourkela

ROOT CAUSE ANALYSIS (why v6 still fails):
==========================================

The BAAI/bge-base-en-v1.5 fallback model produces BERT cosine similarity
of 0.76-0.87 for ALL patent pairs, even completely unrelated ones:
  - "bicycle lock"  → "personal security purse"     : BERT=0.87  ❌
  - "RLHF / LLMs"  → "model referenced worksite"   : BERT=0.81  ❌
  - "bicycle lock"  → "flexible pipe gasket"        : BERT=0.79  ❌

This is called "embedding space collapse" — a general-purpose model maps
all short professional texts into a tight cluster, so cosine similarity
is artificially high for everything.

The threshold (0.385, even raised to 0.45) is still below these inflated
BERT-driven calibrated scores (0.77-0.88), so EVERYTHING gets rejected.

FUNDAMENTAL FIX — v7:
======================

BERT similarity ALONE should NEVER drive a rejection.
Only LEXICAL (TF-IDF) similarity is reliable for cross-domain discrimination.
BERT should only BOOST a decision that TF-IDF already supports.

NEW DECISION HIERARCHY (applied in order):

  Rule 1 — SEMANTIC DRIFT SAFEGUARD (your suggestion #1, MOST IMPORTANT):
    If top_tfidf < 0.15 AND top_bert > 0.75:
       → ACCEPT (Novel — BERT inflated, no lexical match)
    Why: TF-IDF < 0.15 means NO meaningful keyword overlap. BERT > 0.75
         with a general model is just "all patents look similar."
         This single rule fixes RLHF, bicycle lock, and future cases.

  Rule 2 — TF-IDF OVERRIDE (your suggestion #3):
    If max_tfidf_in_results < 0.12 AND max_hybrid < 0.50:
       → ACCEPT (Novel — low lexical similarity across all results)
    Why: Even after fusion, if no patent has meaningful keyword overlap,
         the query is in a new domain.

  Rule 3 — OUT-OF-DOMAIN FLOOR:
    If max_calibrated < novelty_floor (0.40):
       → ACCEPT (Novel — out-of-domain)

  Rule 4 — DUAL-CONDITION REJECTION (v6 logic, kept):
    REJECT only if BOTH:
      max_calibrated >= threshold (0.50)   ← raised from 0.385
      top3_avg_calibrated >= threshold - 0.10
    Otherwise → ACCEPT

  Rule 5 — CONFIDENCE BAND:
    gap = |max_cal - threshold|
    gap > 0.20 → High confidence
    gap > 0.08 → Medium confidence
    else       → Low — manual review recommended

THRESHOLD FIX (your suggestion #2):
  Raised hard minimum threshold from 0.385 to 0.50.
  The learned threshold (BAC-maximising) was 0.415 on a calibrated
  scale where BERT inflates all scores. 0.50 is a safer default that
  accounts for this inflation.

BERT MODEL STATUS:
  anferico/bert-for-patents: fails due to missing protobuf / sentencepiece.
    Fix: pip install protobuf sentencepiece
  AI-Growth-Lab/PatentBERT: 401 Unauthorized — private repo, need HF token.
    Fix: Set HF_TOKEN env var or use: huggingface-cli login
  BAAI/bge-base-en-v1.5: loads fine but is general-purpose.
    The semantic drift safeguard now compensates for its limitations.

  RECOMMENDATION: Install protobuf + sentencepiece for true patent BERT.
    pip install protobuf sentencepiece
  This alone will dramatically improve results without any other changes.

CHANGES IN v7 vs v6:
=====================
FIX-V7-1: Semantic drift safeguard — if tfidf<0.15 and bert>0.75 → ACCEPT
FIX-V7-2: TF-IDF override rule — if max_tfidf<0.12 and hybrid<0.50 → ACCEPT
FIX-V7-3: Threshold hard floor raised 0.385 → 0.50
FIX-V7-4: Novelty floor raised 0.35 → 0.40
FIX-V7-5: Threshold post-training min raised — never go below 0.45
           (sparsity adj now capped at max(0.45, threshold - 0.03))
FIX-V7-6: Decision prints per-result TF-IDF stats for full transparency
FIX-V7-7: protobuf/sentencepiece install hint printed if patent BERT fails
FIX-V7-8: HF_TOKEN hint printed if PatentBERT gives 401
"""
...
# ============================================================
# MODEL PRIORITY LIST
# (model_id, load_method)
#   'hf_bert' → HuggingFace raw encoder + mean-pool wrapper
#   'sbert'   → SentenceTransformer standard loading
# ============================================================
SBERT_MODEL_PRIORITY = [
    ("anferico/bert-for-patents", "hf_bert"),   # Google patent BERT
    ("AI-Growth-Lab/PatentBERT",  "hf_bert"),   # USPTO BERT (needs HF_TOKEN)
    ("BAAI/bge-base-en-v1.5",    "sbert"),      # General — works but inflates BERT
    ("all-MiniLM-L6-v2",         "sbert"),      # Lightweight fallback
]

# ============================================================
# SEMANTIC DRIFT THRESHOLDS (FIX-V7-1, V7-2)
# If using a general-purpose BERT fallback, BERT similarities
# are inflated for ALL pairs. These thresholds detect that case
# and prevent BERT from driving false rejections.
# ============================================================
DRIFT_TFIDF_MAX   = 0.15   # if top TF-IDF below this → possible drift
DRIFT_BERT_MIN    = 0.75   # if BERT above this with low TF-IDF → drift confirmed
TFIDF_OVERRIDE_MAX = 0.12  # if ALL tfidf scores below this → purely BERT-driven
HYBRID_OVERRIDE_MAX = 0.50 # secondary hybrid check for override rule
...
class PatentBERTWrapper:
    ...
    Uses mean pooling over last hidden state + L2 normalisation.
    """
    def __init__(self, model_name, device):
        ...
        print(f"   [PatentBERTWrapper] Loaded {model_name} (HF BERT + mean-pool)")
...
class PatentNoveltySystem:
    """
    Patent Novelty Pre-Screening System — v7.
    Adds semantic drift safeguard and TF-IDF override rules
    to prevent general-purpose BERT from causing false rejections.
    """
    ...
    def __init__(self, cache_dir='cache/', model_dir='models/'):
        ...
        # FIX-V7-3: raised hard floor for threshold
        self.final_threshold = 0.50

        # FIX-V7-4: raised novelty floor
        self.novelty_floor = 0.40

        # FIX-B2: plausibility floor
        ...
        # Track whether we're using a general-purpose fallback model
        # (so we can report it clearly and apply drift safeguards)
    ...
    def build_citation_dataset(self, patent_file, abstract_file, citation_file,
        ...
        abstracts = pd.concat(abs_chunks, ignore_index=True) if abs_chunks else pd.DataFrame()
        ...
        citations = pd.concat(cpairs, ignore_index=True) if cpairs else pd.DataFrame()
        ...
        self.citation_set = set(zip(citations['patent_id'], citations['cited_patent_id']))
        self.citation_set_bidirectional = self.citation_set | {(b, a) for a, b in self.citation_set}
    ...
    def _create_demo_data(self, num_patents=300):
        ...
        domains = {
            'neural_networks':     ['neural network','deep learning','backpropagation','LSTM','transformer','attention','gradient descent','convolutional'],
            'computer_vision':     ['object detection','image segmentation','face recognition','convolution','feature extraction','bounding box','pixel'],
            'nlp':                 ['text classification','sentiment analysis','machine translation','language model','tokenization','named entity','BERT'],
            'reinforcement_learning': ['Q-learning','policy gradient','deep Q network','actor-critic','reward function','Markov','exploration'],
            'optimization':        ['gradient descent','Adam optimizer','learning rate','regularization','hyperparameter','loss function','convergence'],
        }
        patents, pid = [], 1
        ...
        for p in patents:
            for d in domains:
                if d.replace('_', ' ') in p['patent_abstract']:
                    dmap[p['patent_id']] = d; break

        cpairs = []
        pids = df['patent_id'].tolist()
        for i in range(len(pids)):
            for j in range(i+1, len(pids)):
                if dmap.get(pids[i]) == dmap.get(pids[j]) and np.random.random() < 0.25:
    ...
    def _fuse(self, tfidf, bert):
        ...
        If TF-IDF is below floor, BERT is scaled down proportionally.
        This is the core defense against embedding drift.
        """
    ...
    def _load_sbert_model(self):
        ...
        for model_name, method in SBERT_MODEL_PRIORITY:
            try:
                ...
                if method == 'hf_bert':
                    ...
                    _ = model.encode("patent claim", convert_to_tensor=False)
                    ...
                    # Flag if this is a known general-purpose model
                    self._using_general_bert = model_name in (
                        "BAAI/bge-base-en-v1.5", "all-MiniLM-L6-v2")
                    status = "⚠ General-purpose model" if self._using_general_bert else "✓ Patent model"
                    print(f"   {status} loaded: {model_name}")
                    if self._using_general_bert:
                        print(f"   NOTE: Semantic drift safeguards will be applied.")
                        print(f"   TIP: pip install protobuf sentencepiece  "
                              f"→ then re-run to load anferico/bert-for-patents")
                    return model

            except Exception as e:
                msg = str(e)[:100]
                # Helpful hints for known failures
                if 'protobuf' in msg.lower() or 'sentencepiece' in msg.lower():
                    print(f"   ✗ {model_name} — missing library: "
                          f"pip install protobuf sentencepiece")
                elif '401' in msg or 'unauthorized' in msg.lower():
                    print(f"   ✗ {model_name} — private repo, need HF token: "
                          f"set HF_TOKEN env var or run: huggingface-cli login")
                else:
                    print(f"   ✗ {model_name} — {e.__class__.__name__}: {msg}")
                continue

        raise RuntimeError("All BERT models failed.")
    ...
    def _encode_text(self, text_or_list, show_progress_bar=False, batch_size=32):
        ...
        emb = self.sbert_model.encode(
            ...
            device=self.device if self.sbert_model_type == 'sbert' else None,
    ...
    def train_hybrid_model(self, positive_pairs):
        ...
        if ws > 0:
            lw = np.abs(raw_w) / ws
            self.w_tfidf = float(np.clip(lw[0], 0.15, 0.55))
        ...
        print(f"   Score sep (fused): "
              f"pos={fused[pm].mean():.3f} neg={fused[~pm].mean():.3f} "
        ...
        if vla.sum() > 0: print(f"     Pos mean: {vp[vla==1].mean():.3f}")
        if (vla==0).sum()>0: print(f"     Neg mean: {vp[vla==0].mean():.3f}")

        self._optimize_threshold(vp, vla)

        # FIX-V7-5: never go below 0.45 after any adjustment
        if getattr(self, '_sparse_dataset', False):
            old = self.final_threshold
            self.final_threshold = max(0.45, self.final_threshold - 0.03)
            print(f"\n   [Sparsity adj] {old:.3f} → {self.final_threshold:.3f} (floor=0.45)")
    ...
    def _optimize_threshold(self, y_scores, y_true):
        ...
        # FIX-V7-3: hard floor at 0.50 regardless of learned value
    ...
    def compute_hybrid_similarity(self, query_text, top_k=100):
        ...
        if self.sbert_index is not None and FAISS_AVAILABLE:
            ...
            bert_top_idx  = bidx[0]
            bert_scores_d = {int(i): float(np.clip((s+1)/2, 0, 1))
    ...
    def _tfidf_only(self, query_text, top_k=100):
        ...
        return [{'patent_id': self.patent_ids_ordered[i],
                 'title':     self.title_map.get(self.patent_ids_ordered[i], ''),
                 'tfidf_sim': float(sims[i]), 'bert_sim': float(sims[i]),
                 'hybrid_sim': float(sims[i]), 'calibrated_score': float(sims[i])}
                for i in idx]
    ...
    def evaluate_model(self, eval_pairs, eval_top_k=100):
        if eval_pairs is None or len(eval_pairs) == 0:
            print("No eval pairs."); return None
        ...
        print(f"   Model: {self.sbert_model_name}"
              + (" ⚠ General-purpose" if self._using_general_bert else " ✓ Patent-specific"))
        ...
        print(f"\n   CALIBRATED:")
        if n_p: print(f"     Pos: {y_cal[y_true==1].mean():.3f} ± {y_cal[y_true==1].std():.3f}")
        if n_n: print(f"     Neg: {y_cal[y_true==0].mean():.3f} ± {y_cal[y_true==0].std():.3f}")
        if n_p and n_n: print(f"     Sep: {y_cal[y_true==1].mean()-y_cal[y_true==0].mean():.3f}")
        ...
        if not np.isnan(auc): print(f"   AUC-ROC:    {auc:.3f}")
        if not np.isnan(ap):  print(f"   Avg Prec:   {ap:.3f}")
    ...
    def save_cached_embeddings(self):
        ...
        if self.tfidf_matrix_dense is not None:
            np.save(os.path.join(self.cache_dir, 'tfidf_dense.npy'), self.tfidf_matrix_dense)
        meta = {
    ...
    # ============================================================
    # NOVELTY PREDICTION  — v7 decision hierarchy
    # ============================================================

    def predict_novelty(self, new_text, top_k=50):
        """
        v7 Decision Hierarchy (applied in order):

        Rule 1 — Semantic Drift Safeguard:
          If top_tfidf < 0.15 AND top_bert > 0.75 → ACCEPT
          (BERT inflated by general model, no real lexical match)

        Rule 2 — TF-IDF Override:
          If max_tfidf_in_results < 0.12 AND max_hybrid < 0.50 → ACCEPT
          (No meaningful keyword overlap anywhere in results)

        Rule 3 — Out-of-domain Floor:
          If max_calibrated < novelty_floor (0.40) → ACCEPT

        Rule 4 — Dual-condition Rejection:
          REJECT only if max_cal >= threshold AND top3_avg >= threshold-0.10
          Otherwise → ACCEPT
        """
        ...
        top_tfidf   = float(tfidf_scores[0])    # TF-IDF of best match
        top_bert    = float(bert_scores[0])      # BERT of best match
        max_tfidf   = float(max(tfidf_scores))
        max_hybrid  = float(max(r['hybrid_sim'] for r in results))

        # ── Rule 1: Semantic drift safeguard (FIX-V7-1) ──────
        drift_detected = (top_tfidf < DRIFT_TFIDF_MAX and top_bert > DRIFT_BERT_MIN)

        # ── Rule 2: TF-IDF override (FIX-V7-2) ──────────────
        tfidf_override = (max_tfidf < TFIDF_OVERRIDE_MAX and
                          max_hybrid < HYBRID_OVERRIDE_MAX)
        ...
        # ── Apply hierarchy ───────────────────────────────────
        if drift_detected:
            reason    = "BERT Semantic Drift Detected — no lexical match"
            is_novel  = True
        ...
        print(f"   Top-result TF-IDF:       {top_tfidf:.4f}  (key: must be >0.15 for reliable BERT)")
        print(f"   Top-result BERT:         {top_bert:.4f}  (key: >0.75 with low TF-IDF = drift)")
        print(f"   Max TF-IDF (all results):{max_tfidf:.4f}")
        print(f"   Max Hybrid (all results):{max_hybrid:.4f}")

        print(f"\n[Decision Rules]:")
        print(f"   Rule 1 — Drift Safeguard:   {'⚡ TRIGGERED' if drift_detected else '○ not triggered'}"
              f"  [tfidf={top_tfidf:.3f}<0.15 & bert={top_bert:.3f}>0.75]")
        print(f"   Rule 2 — TF-IDF Override:   {'⚡ TRIGGERED' if tfidf_override else '○ not triggered'}"
              f"  [max_tfidf={max_tfidf:.3f}<0.12 & hybrid={max_hybrid:.3f}<0.50]")
        print(f"   Rule 3 — Out-of-domain:     {'⚡ TRIGGERED' if out_of_domain else '○ not triggered'}"
              f"  [max_cal={max_cal:.3f}<floor={self.novelty_floor:.3f}]")
        print(f"   Rule 4 — Dual Rejection:    {'⚡ WOULD REJECT' if dual_reject else '○ no rejection'}"
              f"  [cond1={cond_max}: {max_cal:.3f}≥{self.final_threshold:.3f},"
              f" cond2={cond_top3}: {top3_avg:.3f}≥{self.final_threshold-0.10:.3f}]")

        print(f"\n[Config]:")
        print(f"   Threshold:    {self.final_threshold:.3f}  "
              f"(Drift floor: tfidf<{DRIFT_TFIDF_MAX}, bert>{DRIFT_BERT_MIN})")
        print(f"   Novelty floor:{self.novelty_floor:.3f}  "
              f"(TF-IDF override: <{TFIDF_OVERRIDE_MAX})")
        print(f"   Model:        {self.sbert_model_name}"
              + (" ⚠ [general]" if self._using_general_bert else " ✓ [patent]"))
        print(f"   Weights:      TF-IDF={self.w_tfidf:.3f}, BERT={self.w_bert:.3f}")
        ...
        print(f"  TF-IDF    : {top['tfidf_sim']:.4f}  {'⚠ LOW' if top['tfidf_sim']<0.15 else ''}")
        print(f"  BERT      : {top['bert_sim']:.4f}  "
              f"{'⚠ LIKELY DRIFT' if top['bert_sim']>0.75 and top['tfidf_sim']<0.15 else ''}")
        ...
        for i, r in enumerate(results[:10], 1):
            drift_flag = " ⚠" if r['bert_sim'] > 0.75 and r['tfidf_sim'] < 0.15 else ""
            print(f"\n{i:2d}. {r['patent_id']}  {r['title'][:60]}...")
            print(f"    TF={r['tfidf_sim']:.4f} | BERT={r['bert_sim']:.4f}"
                  f"{drift_flag} | Hybrid={r['hybrid_sim']:.4f} | Cal={r['calibrated_score']:.4f}")
...
def main():
    print("=" * 70)
    print("PATENT NOVELTY CHECK SYSTEM — CORRECTED VERSION v7")
    print("=" * 70)
    print(f"\nAuthor    : Devika Bakshi (122CS0301)")
    ...
    print("─" * 70)
    print("To load anferico/bert-for-patents (recommended for patents):")
    print("  pip install protobuf sentencepiece")
    print("To load AI-Growth-Lab/PatentBERT (needs account):")
    print("  huggingface-cli login  (or set HF_TOKEN env var)")
    print("─" * 70)
    ...
    if changed:
        print(f"\n[INFO] Dataset changed — recomputing")
        ...
        if not system.load_cached_embeddings():
            print("   Cache failed — recomputing...")
            system.compute_embeddings()
            system.train_hybrid_model(positive_pairs)
            system.save_dataset_hash(current_hash)
        else:
    ...
    test_pos['label'] = 1
    test_neg = system.get_mixed_negatives(test_pos, n_test, 0.65, np.random.RandomState(123))
    ...
    # ── Sample predictions ────────────────────────────────────
    # These three cases specifically test the v7 safeguards:
    #
    # Case A: Deep RL + CNN — clear prior art in DB (NN patents abundant)
    #   Expected: REJECT (Rule 4 — both TF-IDF and BERT high)
    #
    # Case B: RLHF/LLM — post-2022, no USPTO prior art
    #   Expected: ACCEPT (Rule 1 or 2 — TF-IDF very low, no lexical match)
    #   With v7: top_tfidf ~0.10-0.14 + bert ~0.80 → drift safeguard fires
    #
    # Case C: Bicycle lock — completely different domain from NN-heavy DB
    #   Expected: ACCEPT (Rule 1 — TF-IDF < 0.15, BERT high due to general model)
    #   With v7: drift safeguard catches this case correctly
    ...
    print("\n" + "=" * 70)
    print("SAMPLE NOVELTY PREDICTIONS — v7 SAFEGUARD TEST")
    print("=" * 70)
    ...
    if system._using_general_bert:
        print(f"  ⚠ Using general-purpose model — semantic drift safeguards active")
        print(f"    → pip install protobuf sentencepiece  to load patent BERT")
    else:
        print(f"  ✓ Patent-specific model — no drift compensation needed")
    print(f"  Threshold         : {system.final_threshold:.3f}")
    print(f"  Novelty floor     : {system.novelty_floor:.3f}")
    print(f"  Plaus. floor      : {system.tfidf_plausibility_floor:.3f}")
    print(f"  Drift safeguard   : tfidf<{DRIFT_TFIDF_MAX} & bert>{DRIFT_BERT_MIN} → ACCEPT")
    print(f"  TF-IDF override   : max_tfidf<{TFIDF_OVERRIDE_MAX} & hybrid<{HYBRID_OVERRIDE_MAX} → ACCEPT")
    ...
    print(f"\n  v7 KEY FIXES:")
    print(f"    FIX-V7-1: Semantic drift safeguard (tfidf<0.15 & bert>0.75 → ACCEPT)")
    print(f"    FIX-V7-2: TF-IDF override rule (max_tfidf<0.12 → ACCEPT)")
    print(f"    FIX-V7-3: Threshold hard floor raised to 0.50")
    print(f"    FIX-V7-4: Novelty floor raised to 0.40")
    print(f"    FIX-V7-5: Sparsity adj floor raised (never below 0.45)")
    print(f"    FIX-V7-6: Full diagnostic output per prediction")
    print(f"    FIX-V7-7: protobuf/sentencepiece install hints")
    print(f"    FIX-V7-8: HF_TOKEN hint for PatentBERT 401 errors")
    print(f"\n  System ready.")
