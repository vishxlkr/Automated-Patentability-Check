...
STABLE VERSION v19 — MPNet Semantic Model + Three-Way Decision

Author: Devika Bakshi (122CS0301)
Supervisor: Asst. Prof. Sumanta Pyne
NIT Rourkela

WHAT v19 FIXES vs v18:
===================================

FIX-V19-1: CASE D (INCREMENTAL) — STEP 9b UPPER BOUND REDESIGNED
  Bug in v18:
    Step 9b added the guard "effective_cal < final_threshold" to prevent
    high-confidence NOT NOVEL patents from mis-labelling as INCREMENTAL.
    This guard was sound in theory but broke in practice because the
    sparsity adjustment dropped final_threshold to 0.400.

    Case D actual v18 scores:
      effective_cal = 0.837    final_threshold = 0.400
      → 0.837 < 0.400 is FALSE → Step 9b never fires
      → Falls through to Rule 5 → [NOT NOVEL]  ✗

  Root cause:
    The guard "cal < threshold" couples Step 9b's firing condition to
    the learned/adjusted threshold. On sparse USPTO data the threshold
    can be as low as 0.40, making the guard impossible to satisfy for
    any non-trivial patent score (typically 0.70–0.95).

  Fix in v19 — ABSOLUTE UPPER CAP:
    Replace "effective_cal < final_threshold" with
            "effective_cal < INCR_CAL_UPPER_ABS" (fixed at 0.92)

    This decouples the guard from the threshold entirely.
    The 0.92 cap still blocks genuinely very-high-confidence matches
    (where NOT NOVEL is almost certain), while allowing the incremental
    zone to fire for mid-to-high cal scores like Case D's 0.837.

    Verification against all four cases (from v18 USPTO run):
      Case A: gap=0.288 < 0.40  → FAILS cond_a → NOT NOVEL ✓
              tfidf=0.468 > 0.28 → FAILS cond_b too (double-safe)
              sem=0.756 > 0.74   → FAILS cond_c too (triple-safe)
      Case B: Step 1 (ModernTerms) fires first → NOVEL ✓
      Case C: Step 2 (SemanticGap) fires first → NOVEL ✓
      Case D: gap=0.479 ✓  tfidf=0.229 ✓  sem=0.709 < 0.74 ✓
              cal=0.837 >= 0.36 ✓  cal=0.837 < 0.92 ✓
              → INCREMENTAL ✓

FIX-V19-2: NOVELTY_FLOOR TIED TO FINAL_THRESHOLD
  v18 kept novelty_floor=0.400 unconditionally, which equalled
  final_threshold on sparse data (both 0.400). This meant Rule 4
  (out-of-domain) and Rule 5 (dual-reject) had the same trigger,
  making Rule 4 redundant. v19 sets:
    novelty_floor = max(0.30, final_threshold * 0.75)
  This restores the meaningful gap between the floor and threshold.

FIX-V19-3: CACHE VERSION BUMPED TO v19
  All v18/v17/v16 caches auto-invalidate.

DECISION HIERARCHY v19 (structure unchanged, Step 9b guard fixed):
================================================================
...
Step 9b  Incremental (gap+tfidf+sem guard) → INCREMENTAL  ← FIXED v19
...
Expected results:
  ...
  Case D (CNN + attention, marginal)   → INCREMENTAL  ✓ (fixed v19)
"""
...
THRESHOLDS = {
    ...
    # v19: Gap+TF-IDF+Semantic Window (Step 9b — fixed)
    #
    # Key change from v18: "INCR_CAL_UPPER_ABS" replaces the buggy
    # "effective_cal < final_threshold" guard.
    #
    # v18 bug: when final_threshold=0.400 (sparse data sparsity adj),
    #   Case D cal=0.837 fails 0.837 < 0.400 → Step 9b never fired.
    #
    # v19 fix: use a fixed absolute cap (0.92) independent of threshold.
    #   Case D: 0.837 < 0.92 ✓ → fires correctly
    #   Very-high-confidence NOT NOVEL (cal > 0.92): still blocked ✓
    #
    # Verification (v18 USPTO scores):
    #   Case A: gap=0.288 < 0.40     → FAILS cond_a → NOT NOVEL ✓
    #           tfidf=0.468 > 0.28   → FAILS cond_b (extra safety) ✓
    #           sem=0.756 > 0.74     → FAILS cond_c (extra safety) ✓
    #   Case B: exits Step 1 (7 modern terms) → NOVEL ✓
    #   Case C: exits Step 2 (gap=0.615>0.55) → NOVEL ✓
    #   Case D: gap=0.479 ✓  tfidf=0.229 ✓  sem=0.709<0.74 ✓
    #           cal=0.837 >= 0.36 ✓  cal=0.837 < 0.92 ✓  → INCREMENTAL ✓
    ...
    "INCR_SEM_MAX":                  0.74,   # semantic upper bound
    "INCR_CAL_UPPER_ABS":            0.92,   # NEW v19: absolute cal upper cap
                                             # replaces threshold-relative guard
}

# ============================================================
# MODERN TERMINOLOGY VOCABULARY (post-2022 AI/ML terms)
# Expanded in v18 with 2024-2025 terms, carried forward to v19
# ============================================================
...
class PatentNoveltySystem:
    """
    Hybrid Patent Novelty Detection System — v19
    ...
    Architecture:
      ...
      - Gap+TF-IDF+Semantic window (v19): fixed incremental heuristic
    ...
    Key fix in v19:
      Step 9b now uses INCR_CAL_UPPER_ABS (0.92, fixed) instead of
      final_threshold as the upper cal bound. This prevents the sparsity-
      adjusted threshold (can be as low as 0.40) from making the guard
      impossible to satisfy for realistic patent scores (0.70-0.92).
    """
    ...
    def _combined_sim(self, tfidf_s, semantic_s):
        """Hybrid score for negative mining candidate selection."""
    ...
    # ============================================================
    # NEGATIVE MINING — hybrid-aware three-tier (from v18)
    # ============================================================
    ...
    def get_hard_negatives(self, positive_pairs, target, lo_pct=0.50, hi_pct=0.80,
        ...
        TF-IDF + semantic score to surface genuinely ambiguous pairs.
        """
    ...
    def train_hybrid_model(self, positive_pairs):
        ...
        # v18/v19: floor 0.42 (normal), 0.40 (sparse), adj = -0.02
        ...
        # v19: novelty_floor tied to final_threshold (not a fixed 0.40)
        # This ensures Rule 4 remains distinct from Rule 5 even on sparse data.
        self.novelty_floor = max(0.30, self.final_threshold * 0.75)
        print(f"   [v19] novelty_floor set to max(0.30, {self.final_threshold:.3f}×0.75)"
              f" = {self.novelty_floor:.3f}")
    ...
    def _learn_incremental_thresholds(self, tt, st, yl, vt, vs, vl, vp, vla):
        print(f"\n[v19] Learning incremental zone thresholds from positive pair distribution...")
        ...
        # Clamp at 0.90 max (v18 improvement) to ensure zone width >= 0.04
        ...
        print(f"     incr_fraction        = {incr_fraction:.3f}  (clamped at 0.90 max)")
        ...
        print(f"\n   Step 9b config (v19):")
        print(f"     INCR_CAL_UPPER_ABS   = {self._thr['INCR_CAL_UPPER_ABS']:.3f}  "
              f"(absolute upper cap — decoupled from threshold)")

    # ============================================================
    # RETRIEVAL — enlarged candidate pool (from v18)
    # ============================================================
    ...
    def evaluate_model(self, eval_pairs, eval_top_k=100):
        ...
        print(f"   [v19] INCREMENTAL counts as NOVEL for binary classification metrics")
    ...
    def plot_hybrid_similarity_distribution(self, eval_pairs,
        ...
        ax.set_title(f'Similarity Distribution — Hybrid MPNet System (v19)\n'
    ...
    def load_cached_embeddings(self):
        ...
        try:
            ...
            logger.info(f"Cache loaded (v19, model={SEMANTIC_MODEL_NAME})")
            return True
        except Exception as e:
    ...
    # ============================================================
    # NOVELTY PREDICTION — v19 decision hierarchy
    # ============================================================

    def predict_novelty(self, new_text, top_k=50):
        """
        v19 Decision Hierarchy:
        ...
        Key v19 change in Step 9b:
          OLD (v18): effective_cal < final_threshold
                     Breaks when threshold=0.40 and Case D cal=0.837
          NEW (v19): effective_cal < INCR_CAL_UPPER_ABS (fixed=0.92)
                     Works regardless of threshold value
        """
        if self.patents_df is None:
            raise ValueError("Dataset not loaded.")

        print("\n" + "=" * 70)
        print("NOVELTY PREDICTION — Hybrid TF-IDF + MPNet System v19")
        print("=" * 70)
        ...
        # ── Step 9b: Incremental — Gap+TF-IDF+Semantic Window (v19) ───
        #
        # v19 FIX: Replaced "effective_cal < final_threshold" with
        #          "effective_cal < INCR_CAL_UPPER_ABS" (fixed = 0.92)
        #
        # WHY: In v18, on sparse USPTO data, final_threshold can be as
        # low as 0.400 after sparsity adjustment. Case D's cal=0.837
        # cannot satisfy 0.837 < 0.400, so Step 9b never fired.
        # The absolute cap 0.92 is independent of the learned threshold
        # and correctly allows Case D (cal=0.837) while still blocking
        # extremely high-confidence matches (cal >= 0.92).
        #
        # Verification against all four cases (from v18 USPTO run):
        #   Case A: gap=0.288 < 0.40   → FAILS cond_a → NOT NOVEL ✓
        #           tfidf=0.468 > 0.28  → FAILS cond_b (extra safety) ✓
        #           sem=0.756 > 0.74    → FAILS cond_c (extra safety) ✓
        #   Case B: Step 1 fires first  → NOVEL ✓
        #   Case C: Step 2 fires first  → NOVEL ✓
        #   Case D: gap=0.479 ✓  tfidf=0.229 ✓  sem=0.709 < 0.74 ✓
        #           cal=0.837 >= 0.36 ✓  cal=0.837 < 0.92 ✓
        #           → INCREMENTAL ✓
        incremental_gap_window = (
            ...
            top_semantic < T["INCR_SEM_MAX"]                            and
            effective_cal >= T["INCREMENTAL_CAL_LOW"]                   and
            effective_cal < T["INCR_CAL_UPPER_ABS"]                          # v19: FIXED
        )
        ...
        if modern_override:
            ...
            reason  = (
                ...
                f"Cal ({effective_cal:.3f}) < {T['INCR_CAL_UPPER_ABS']} (absolute cap). "
                f"Manual examiner review recommended."
            )
            decision   = "[INCREMENTAL] Incremental Innovation — Improvement Patent"
        ...
        print(f"   Step 9b — Incremental(gap+tfidf+sem window) [v19 FIXED]: "
              f"{'⚡ → INCREMENTAL' if incremental_gap_window else '○ skip'}")
        print(f"             [gap={semantic_gap:.3f} in [{T['INCR_GAP_LOW']},{T['INCR_GAP_HIGH']})"
              f" & tfidf={max_tfidf:.3f} in [{T['INCR_TFIDF_LO2']},{T['INCR_TFIDF_HI2']}]"
              f" & sem={top_semantic:.3f} < {T['INCR_SEM_MAX']}"
              f" & eff_cal={effective_cal:.3f} in [{T['INCREMENTAL_CAL_LOW']:.3f},"
              f"{T['INCR_CAL_UPPER_ABS']})]")
        print(f"             [v19: upper bound = INCR_CAL_UPPER_ABS={T['INCR_CAL_UPPER_ABS']}"
              f" (fixed, not threshold-relative)]")
...
def main():
    print("=" * 70)
    print("HYBRID PATENT NOVELTY DETECTION SYSTEM — v19")
    ...
    print(f"  → v19: Step 9b upper bound fixed (absolute cap, not threshold-relative)")
    ...
    print("\n" + "=" * 70)
    print("SAMPLE NOVELTY PREDICTIONS — v19")
    print("=" * 70)
    ...
    print(f"\n{'='*70}")
    print(f"FINAL SUMMARY — Hybrid Patent Novelty Detection System v19")
    print(f"{'='*70}")
    ...
    print(f"    Step 9b  Incremental(gap+tfidf+sem window): ← FIXED v19")
    print(f"             gap in [{T['INCR_GAP_LOW']},{T['INCR_GAP_HIGH']})"
          f" & tfidf in [{T['INCR_TFIDF_LO2']},{T['INCR_TFIDF_HI2']}]"
          f" & sem < {T['INCR_SEM_MAX']}")
    print(f"             & eff_cal in [{T['INCREMENTAL_CAL_LOW']:.3f},{T['INCR_CAL_UPPER_ABS']})"
          f" → INCREMENTAL")
    print(f"             [v19: upper={T['INCR_CAL_UPPER_ABS']} fixed, not threshold-relative]")
    ...
    print(f"\n  v19 fixes vs v18:")
    print(f"    FIX-V19-1: Step 9b upper bound — absolute cap replaces threshold-relative guard")
    print(f"      v18 bug:  effective_cal < final_threshold (= 0.400 on sparse data)")
    print(f"                Case D cal=0.837: 0.837 < 0.400 = False → Step 9b never fired")
    print(f"                → wrongly fell through to Rule 5 → [NOT NOVEL] ✗")
    print(f"      v19 fix:  effective_cal < INCR_CAL_UPPER_ABS (= {T['INCR_CAL_UPPER_ABS']})")
    print(f"                Case D cal=0.837: 0.837 < {T['INCR_CAL_UPPER_ABS']} = True → ✓ INCREMENTAL")
    print(f"                Case A: fails cond_a (gap<0.40) AND cond_b (tfidf>0.28) → NOT NOVEL ✓")
    print(f"    FIX-V19-2: novelty_floor = max(0.30, threshold×0.75) = {system.novelty_floor:.3f}")
    print(f"               Restores meaningful gap between floor and threshold on sparse data")
    print(f"    FIX-V19-3: Cache version bumped to v19 (v18/v17 caches auto-invalidate)")
    print(f"    Retained from v18: hybrid negative mining, enlarged candidate pools,")
    print(f"               expanded modern terms vocabulary (2024-2025)")
    print(f"\n  System ready.")
