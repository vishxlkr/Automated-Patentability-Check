...
STABLE VERSION v14 — MPNet Semantic Model + Three-Way Decision (FIXED)
...
WHAT v14 FIXES vs v13:
=======================

FIX-V14-1: IMPOSSIBLE INCREMENTAL INTERVAL — ROOT CAUSE + FIX
  Bug in v13:
    _learn_incremental_thresholds() ran BEFORE the sparsity adjustment block.
    Sparsity adj lowered final_threshold from 0.500 → 0.470 AFTER the function ran.
    INCREMENTAL_CAL_LOW was clamped to (0.500 - 0.02) = 0.480 inside the function.
    After sparsity adj: interval became [0.480, 0.470) — mathematically impossible.
    Step 9 never activated.

  Fix in v14:
    _learn_incremental_thresholds() is now called AFTER all threshold adjustments
    (including sparsity adj). final_threshold is fully settled before we compute
    INCREMENTAL_CAL_LOW. The lower bound is always set to final_threshold * fraction,
    guaranteeing final_threshold * fraction < final_threshold.

FIX-V14-2: PERCENTILE DISPLAY BUG
  Bug in v13:
    p75 in the Cal(val) printout showed the post-clamp value (0.480)
    while p25 and p50 showed raw values (0.516, 0.706) → impossible ordering.
    This was only a display issue but caused confusion about whether the
    percentile calculation itself was broken.

  Fix in v14:
    Raw p75 is saved separately before clamping. All four percentiles
    (p25, p50, p75_raw, p75_clamped) are printed clearly.

FIX-V14-3: GAP CONDITION REDESIGNED
  Bug in v13:
    Used median gap of positive pairs (~0.654) as the incremental gap ceiling.
    Positive pairs in this dataset have HIGH semantic gaps because MPNet embeds
    the semantic meaning deeply — even related patents differ significantly in
    surface wording. So gap < 0.654 was always true and not discriminative.

  Fix in v14:
    Incremental zone uses a FIXED gap ceiling of 0.45 (not data-driven from gap).
    Rationale: if gap > 0.45, the query patent has drifted significantly from
    the retrieved prior art in embedding space. That is a novelty signal, not
    an incremental improvement signal. The previous rules (gap override, drift
    safeguard) already handle large-gap cases upstream. By the time we reach
    Step 9, gap must be moderate to qualify as incremental.
    The gap ceiling is intentionally NOT learned from data because gap in the
    dataset reflects the corpus distribution, not a principled boundary.

FIX-V14-4: TERMINOLOGY UPDATED THROUGHOUT
  Old:  [ACCEPT] Potentially Novel
        [BORDERLINE] Incremental Innovation
        [REJECT] Not Novel — Prior Art Detected

  New:  [NOVEL] Potentially Novel
        [INCREMENTAL] Incremental Innovation — Improvement Patent
        [NOT NOVEL] Prior Art Detected

FIX-V14-5: INCREMENTAL LOWER BOUND IS fraction × final_threshold
  Instead of a percentile-clamped absolute value, we now derive:
    INCREMENTAL_CAL_LOW = final_threshold * incr_fraction
  where incr_fraction = position of the 70th percentile of positive calibrated
  scores as a fraction of final_threshold, clamped to [0.80, 0.96].
  This guarantees: INCREMENTAL_CAL_LOW < final_threshold always.

DECISION HIERARCHY v14 (unchanged logic, fixed Step 9):
=========================================================
...
Step 9  — Incremental Innovation       → INCREMENTAL  ← FIXED
...
Expected results:
  ...
  Case D (CNN + attention, marginal)   → INCREMENTAL  ✓ (fixed)
"""
...
THRESHOLDS = {
    ...
    # Incremental zone — initial defaults; all overwritten dynamically after training.
    # INCREMENTAL_CAL_LOW   = final_threshold × incr_fraction  (always < final_threshold)
    # INCREMENTAL_CAL_HIGH  = final_threshold                  (upper boundary = REJECT)
    # INCREMENTAL_TFIDF_LOW = 75th pct of positive TF-IDF scores (real lexical overlap)
    # INCREMENTAL_TFIDF_HIGH= 90th pct of positive TF-IDF scores
    # INCREMENTAL_GAP_MAX   = 0.45 (fixed — see FIX-V14-3)
    "INCREMENTAL_CAL_LOW":           0.45,   # overwritten; always < final_threshold
    "INCREMENTAL_TFIDF_LOW":         0.15,   # overwritten by 75th pct of pos TF-IDF
    "INCREMENTAL_TFIDF_HIGH":        0.40,   # overwritten by 90th pct of pos TF-IDF
    "INCREMENTAL_GAP_MAX":           0.45,   # fixed — not learned (see FIX-V14-3)
    "INCREMENTAL_TOP3_LOW":          0.45,   # overwritten; tracks INCREMENTAL_CAL_LOW
}
...
class PatentNoveltySystem:
    """
    Hybrid Patent Novelty Detection System — v14
    ...
    Architecture:
      ...
      - [v13/v14] Incremental zone: data-driven INCREMENTAL decision
        for patents closely related to prior art but showing improvement pattern
    ...
    def __init__(self, cache_dir='cache/', model_dir='models/'):
        ...
        # v14: flag — True once _learn_incremental_thresholds has run
    ...
    def train_hybrid_model(self, positive_pairs):
        ...
        # ── Sparsity adjustment — runs BEFORE incremental learning ─────────────────
        # CRITICAL ORDER: sparsity adj must settle final_threshold BEFORE
        # _learn_incremental_thresholds() reads it. This was the root cause of
        # the impossible interval bug in v13.
        ...
        # ── v14: Learn incremental zone thresholds AFTER final_threshold is settled ─
    ...
    def _learn_incremental_thresholds(self, tt, st, yl, vt, vs, vl, vp, vla):
        """
        v14: Derive incremental zone boundaries from citation-positive pair distributions.

        KEY FIX vs v13:
        - Called AFTER all threshold adjustments (including sparsity adj).
          final_threshold is fully settled when this runs.
        - INCREMENTAL_CAL_LOW is stored as final_threshold × incr_fraction.
          This GUARANTEES the interval [INCREMENTAL_CAL_LOW, final_threshold) is valid.
        - incr_fraction is derived from data (70th pct of positive cal scores,
          expressed as fraction of final_threshold), clamped to [0.80, 0.96].
        - Gap ceiling is FIXED at 0.45 (not learned — see FIX-V14-3 in docstring).
        - TFIDF bounds are still learned from data (75th and 90th percentiles).

        Incremental zone definition (all conditions must hold):
          1. INCREMENTAL_CAL_LOW <= effective_cal < final_threshold
             → score is in the upper tail of positive pairs, but below REJECT
          2. INCREMENTAL_TFIDF_LOW <= max_tfidf <= INCREMENTAL_TFIDF_HIGH
             → meaningful lexical overlap — same technical field
          3. semantic_gap < INCREMENTAL_GAP_MAX (= 0.45 fixed)
             → not a pure semantic drift — patent is lexically grounded in the field
          4. top3_avg >= INCREMENTAL_TOP3_LOW
             → consistent top-3 similarity (not a single spurious match)
        """
        print(f"\n[v14] Learning incremental zone thresholds from positive pair distribution...")
        print(f"   [Note] Called after sparsity adj — final_threshold = {self.final_threshold:.3f}")

        # ── Collect positive pair scores (train + val) ─────────────────────────────
        ...
        val_pos_mask = np.array(vl) == 1          # boolean mask for val positives
        ...
        # FIX: vp and vla are both indexed 0..N-1, aligned by the same val_all loop
        val_pos_cal      = vp[vla == 1]            # calibrated scores for val positives

        # Combine train and val positives for better statistics
        ...
        tfidf_p75 = max(tfidf_p75, 0.08)   # floor: must be non-trivial

        # ── Calibrated score bound (data-driven fraction) ──────────────────────────
        # We use the 70th percentile of ALL positive calibrated scores (train + val).
        # This is the lower boundary of the "very similar positives" region.
        # We store it as a fraction of final_threshold so the interval is always valid.
        ...
        # Express as fraction of final_threshold, clamp to [0.80, 0.96]
        # 0.80 → lower bound is 80% of threshold (wide incremental zone)
        # 0.96 → lower bound is 96% of threshold (narrow incremental zone)
        if self.final_threshold > 0:
            raw_fraction = cal_p70_raw / self.final_threshold
        else:
            raw_fraction = 0.88
        incr_fraction = float(np.clip(raw_fraction, 0.80, 0.96))
        cal_low = self.final_threshold * incr_fraction   # ALWAYS < final_threshold

        # ── Gap ceiling is fixed (not data-driven) — see FIX-V14-3 ─────────────────
        gap_max_fixed = self._thr["INCREMENTAL_GAP_MAX"]  # = 0.45
        ...
        self._thr["INCREMENTAL_TOP3_LOW"]   = cal_low   # same lower boundary for top3

        self._incremental_thresholds_learned = True

        # ── Print diagnostics (raw percentiles shown before any clamping) ──────────
        ...
        print(f"     Cal(all pos) — p25={cal_p25_raw:.3f}  p50={cal_p50_raw:.3f}"
              f"  p70={cal_p70_raw:.3f}  (raw, before fraction mapping)")

        print(f"\n   Incremental zone thresholds (learned):")
        print(f"     incr_fraction        = {incr_fraction:.3f}  "
              f"(raw_fraction={raw_fraction:.3f}, clamped to [0.80, 0.96])")
        print(f"     INCREMENTAL_CAL_LOW  = final_threshold × fraction"
              f" = {self.final_threshold:.3f} × {incr_fraction:.3f} = {cal_low:.3f}")
        print(f"     INCREMENTAL_CAL_HIGH = final_threshold = {self.final_threshold:.3f}")
        print(f"     INCREMENTAL_TFIDF_LOW  = {tfidf_p75:.3f}  (75th pct of pos TF-IDF)")
        print(f"     INCREMENTAL_TFIDF_HIGH = {tfidf_p90:.3f}  (90th pct of pos TF-IDF)")
        print(f"     INCREMENTAL_GAP_MAX    = {gap_max_fixed:.3f}  (fixed — not learned)")

        print(f"\n   Incremental zone interpretation:")
        print(f"     eff_cal in [{cal_low:.3f}, {self.final_threshold:.3f})  "
              f"← interval is valid: {cal_low:.3f} < {self.final_threshold:.3f}")
        print(f"     AND max_tfidf in [{tfidf_p75:.3f}, {tfidf_p90:.3f}]")
        print(f"     AND semantic_gap < {gap_max_fixed:.3f}  (moderate domain proximity)")
        print(f"     AND top3_avg >= {cal_low:.3f}")
    ...
    def evaluate_model(self, eval_pairs, eval_top_k=100):
        ...
        print(f"   [v14] INCREMENTAL counts as NOVEL for binary classification metrics")
    ...
    def load_cached_embeddings(self):
        ...
        try:
            ...
            logger.info(f"Cache loaded (v14, model={SEMANTIC_MODEL_NAME})")
            return True
        except Exception as e:
    ...
    # ============================================================
    # NOVELTY PREDICTION — v14 decision hierarchy
    # ============================================================

    def predict_novelty(self, new_text, top_k=50):
        """
        v14 Decision Hierarchy:
        ...
        Step 7:  Rule 3 — TF-IDF Override     → [NOVEL]
        Step 8:  Rule 4 — Out-of-domain Floor → [NOVEL]
        Step 9:  Incremental Innovation       → [INCREMENTAL]   ← FIXED in v14
        Step 10: Rule 5 — Dual Rejection      → [NOT NOVEL]
        Step 11: Confidence Band              → [NOVEL]
        """
        ...
        print("\n" + "=" * 70)
        print("NOVELTY PREDICTION — Hybrid TF-IDF + MPNet System v14")
        print("=" * 70)
        ...
        # ── Step 9: Incremental Innovation (v14 — FIXED) ──────────────
        #
        # This fires when the query is in the UPPER TAIL of the positive pair
        # score distribution but has NOT crossed the full REJECT threshold.
        # The four conditions jointly identify the "improvement patent" fingerprint:
        #
        #   1. Score is in the incremental zone:
        #      INCREMENTAL_CAL_LOW <= effective_cal < final_threshold
        #      → high similarity (above 80–96% of threshold) but not rejected
        #      → INCREMENTAL_CAL_LOW is always < final_threshold (guaranteed by design)
        #
        #   2. TF-IDF is in the meaningful overlap band:
        #      INCREMENTAL_TFIDF_LOW <= max_tfidf <= INCREMENTAL_TFIDF_HIGH
        #      → same technical field (not noise, not copying)
        #
        #   3. Semantic gap is moderate:
        #      semantic_gap < INCREMENTAL_GAP_MAX (= 0.45, fixed)
        #      → patent is lexically grounded in the same domain
        #      → large gaps are handled by Step 2/3/5 upstream (novel by drift)
        #
        #   4. Top-3 average is also high (consistent match, not one spurious hit):
        #      top3_avg >= INCREMENTAL_TOP3_LOW
        #
        incremental_patent = (
        ...
        if modern_override:
            ...
            # Incremental improvement patent:
            # Score is in the high-tail zone of citation-positive pairs,
            # TF-IDF shows meaningful same-field lexical overlap,
            # semantic gap is moderate (not drifted, not copying),
            # and the match is consistent across top-3 results.
            gap_to_threshold = self.final_threshold - effective_cal
            reason  = (
                f"Incremental Innovation Pattern — calibrated score "
                f"({effective_cal:.3f}) is in the zone "
                f"[{T['INCREMENTAL_CAL_LOW']:.3f}, {self.final_threshold:.3f}) "
                f"with TF-IDF overlap ({max_tfidf:.3f}) indicating same technical field "
                f"and moderate semantic gap ({semantic_gap:.3f} < {T['INCREMENTAL_GAP_MAX']:.3f}). "
                f"Score fingerprint matches improvement / optimisation patents rather than "
                f"direct duplication ({gap_to_threshold:.3f} below rejection threshold). "
        ...
        if self._incremental_thresholds_learned:
            ...
            print(f"   Step 9 — Incremental:        "
                  ...
                  f"{self.final_threshold:.3f})  ← interval {'✓ valid' if interval_ok else '✗ BUG'}"
...
def main():
    print("=" * 70)
    print("HYBRID PATENT NOVELTY DETECTION SYSTEM — v14")
    ...
    print(f"  → v14: Incremental zone fixed — INCREMENTAL_CAL_LOW always < final_threshold")
    ...
    print("\n" + "=" * 70)
    print("SAMPLE NOVELTY PREDICTIONS — v14")
    print("=" * 70)
    ...
    T = system._thr
    print(f"\n{'='*70}")
    print(f"FINAL SUMMARY — Hybrid Patent Novelty Detection System v14")
    print(f"{'='*70}")
    ...
    print(f"    Rule 2  Coherence:    median_top5<{T['COHERENCE_TFIDF_MEDIAN']} → NOVEL")
    ...
    if system._incremental_thresholds_learned:
        ...
        print(f"    Step 9  Incremental:  eff_cal in [{T['INCREMENTAL_CAL_LOW']:.3f},"
              f"{system.final_threshold:.3f})  ← {'✓ valid' if interval_ok else '✗ BUG'}")
        print(f"                          & tfidf in [{T['INCREMENTAL_TFIDF_LOW']:.3f},"
              f"{T['INCREMENTAL_TFIDF_HIGH']:.3f}]")
        print(f"                          & gap < {T['INCREMENTAL_GAP_MAX']:.3f} (fixed)")
        print(f"                          & top3 >= {T['INCREMENTAL_TOP3_LOW']:.3f}"
              f" → INCREMENTAL")
    else:
        print(f"    Step 9  Incremental:  [thresholds not learned yet]")
    ...
    print(f"\n  v14 fixes vs v13:")
    print(f"    FIX-V14-1: Impossible interval — sparsity adj now runs BEFORE")
    print(f"               _learn_incremental_thresholds(), so final_threshold is")
    print(f"               settled when INCREMENTAL_CAL_LOW is computed.")
    print(f"               INCREMENTAL_CAL_LOW = final_threshold × {system._thr.get('INCREMENTAL_CAL_LOW', 0) / system.final_threshold if system.final_threshold else 0:.3f}")
    print(f"               always < final_threshold (guaranteed by design).")
    print(f"    FIX-V14-2: Percentile display bug fixed — raw p75 shown before clamping.")
    print(f"    FIX-V14-3: Gap ceiling changed from data-driven (median ~0.65) to")
    print(f"               fixed 0.45 — prevents the condition from being trivially true.")
    print(f"    FIX-V14-4: Terminology: NOVEL / INCREMENTAL / NOT NOVEL throughout.")
    print(f"    FIX-V14-5: Cache version bumped to v14 — old caches auto-recompute.")
    print(f"\n  System ready.")
